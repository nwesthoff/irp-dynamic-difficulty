...
Experiences like this are driven by _intrinsic motivation_, which means we are motivated because we find a task enjoyable. On the opposite, extrinsic motivation means we are motivated by an external incentive (like a reward). Mark Brown, from _Game Maker's Toolkit_, motivates why it's so important to know what motivates players to play a game <Citation id="" hideAuthor />.
...
Mark Brown does have a fairly black-and-white perspective on this issue. Contrarily, Jesse Schell discusses several theories that show how the lines between extrinsic and intrinsic motivation blur when applied to the context of gaming <Citation id="" hideAuthor />.
...
He views the complexity of intrinsic and extrinsic as a continuous gradient, something that isn't as binary as Mark Brown sees it.

Adding extrinsic motivation to something that is already intrinsically motivating moves it on the continuum towards external, taking away the previous intrinsic motivation.

![/img/motivation-table.png](/img/motivation-table.png)

Balancing this idea is especially difficult for escape rooms because of its fixed time limit. On the one hand, we don't want players to finish the escape room with half an hour on the clock. On the other hand, we don't want to make the experience _so_ long almost no player gets to experience the conclusion (remember fair puzzles, how about fair escape rooms).
