...
Ermi and Mäyrä categorise immersion in (video) games in three different types <Citation id="nacke_2008_flow" />.
...
That's the big difference between real spaces, that people actually live in, and an escape room. Escape rooms tend to feel like an empty husk. They have more likeness to something you would see in the photos of an architecture portfolio than the workspace of someone like Nikola Tesla. Each object in an escape room has its purpose, and is heavily curated.
...
Escape rooms offer a limited amount of intractable objects for a reason. If the escape room would be in the workshop, times would be spent mostly on identifying the puzzles, not actually solving them.

Victor has an interesting perspective on the matter as well.
